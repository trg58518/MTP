...
do_config_mtp() {
    ...
    while true; do
        default_provider=2
        read -p "(按下回车开始安装):" input_provider
    ...
    while true; do
        default_port=443
        read -p "(请输入一个客户端连接端口: ${default_port}):" input_port
    ...
    while true; do
        default_manage=8888
        read -p "请输入一个管理端口 ${default_manage}: " input_manage_port
    ...
    while true; do
        ...
        read -p "请输入一个需要伪装的域名：${default_domain}" input_domain
