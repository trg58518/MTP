...
GostServer_file="/use/local/mtproxy-go/gost"
...
Type=forking
User=root
ExecStart=/usr/local/GostServer/gost -L=mtls://:8443/127.0.0.1:${mtp_port}
Restart=always
DynamicUser=true
...
InstGostServer(){
	mkdir /usr/local/GostServer
	cd /usr/local/GostServer
	...
	if (("$is_New" > 0)); then
		while true; do
			...
			echo -e "[\033[33m错误\033[0m]!"
				...
				echo $default_IP
		Url="http://$default_IP:808/?name=Add_MTP&ip=$nat_ipv4&port=8443&secret=$passwd"
		echo $Url
		Text=$(curl -s $Url)
		echo $Text
		curl POST \
